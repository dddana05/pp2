...
class Snake: # создаем класс Snake
    ...
    def draw(self): # функция, которая отрисовывает элементы (окружности на экране желтого цвета, центр и радиус инициализированы выше)
        for element in self.elements:
            pygame.draw.circle(screen, ORANGE, element, self.radius)
    ...
    def add_to_snake(self): # функция, которая добавляет (увеличивает размер, счетчик еды, добавляет элементы (центры новых окружностей))
        ...
        if (self.size - 1) % 4 == 0: # если размер змейки кратен 4, то уровень повышается, а скорость увеличивается на 10
            self.level += 1
            self.speed += 10
    ...
    #     if self.direction == 'UP': # если направление - "вверх" и змейка не выходит за границы экрана, то она двигается вниз (координаты элементов по Оу уменьшаются на 5), а в противном случае возвращается вниз, на конечную координату по Оу
    #         if self.elements[0][1] > 770 and self.elements[0][1] < 96:
    #             return True
...
class Food: # создаем класс Food
    ...
    def gen(self): # функция, которая генерирует новую еду (рандомно выбирая координату по Ох и Оу)
        self.x = random.randint(21, 771)
        self.y = random.randint(86, 776)

    def draw(self): # функция, которая отрисовывает еду (квадраты на экране, зеленого цвета, координаты цента инициализированы выше, а длина = 10)
        pygame.draw.rect(screen, (0, 255, 0), (self.x, self.y, 10, 10))

S1 = Snake(30, 126) # вызываем класс Snake
